...
def square_wave_mechanism(poi_idx, R, epsilon):
    ...
    t = R / delta
    b = (epsilon * (np.e ** (epsilon)) - np.e ** epsilon + 1) / (2 * np.e ** (epsilon) * (np.e ** epsilon - 1 - epsilon))
...
def post_process_for_R(traj_center_nearest_poi, perturbed_R, epsilon_for_d1):
    ...
    b = (epsilon_for_d1 * (np.e ** (epsilon_for_d1)) - np.e ** epsilon_for_d1 + 1) / (2 * np.e ** (epsilon_for_d1) * (np.e ** epsilon_for_d1 - 1 - epsilon_for_d1))
    ...
    for t_idx, t in enumerate(t_list):
        d = (2 * b * (np.e ** (epsilon_for_d1) - 1) * t) / (2 * b * np.e ** epsilon_for_d1 + 1) + (1 + 2 * b) / (2 * (2 * b * np.e ** (epsilon_for_d1) + 1)) - t
        v = b ** 2 / 3 + ((2 * b + 1) * (b + 1 - 3 * t ** 2)) / (3 * (2 * b * np.e ** epsilon_for_d1 + 1)) - d ** 2 - 2 * d * t
    ...
    if len(res_1_list)>0:
        t_star_cali = (sum(res_1_list) + sum(res_2_list))\
             / (len(res_1_list) * np.e ** epsilon_for_d1 / (2 * b * np.e ** epsilon_for_d1 + 1) + len(res_2_list) * 1 / (2 * b * np.e ** epsilon_for_d1 + 1))
    else:
...
def ATP_mechanism(traj, epsilon, poi_angel_num_matrix_adap, poi_angel_num_adap_list, epsilon_direct_rate=0.75, kRR_exp_kRR=False, traj_idx=None):
    ...
    if R < avg_dist:
        ...
        R = R - (R - avg_dist) * np.e ** ( - epsilon_for_d1) * sigmoid((R - avg_dist) / (2 * (max(poi_distance_matrix[traj_center_nearest_poi]) - avg_dist)))
    ...
    if len(traj_array)%2==1:
        for i, this_poi in enumerate(poi_list_for_direc_dist):
            if kRR_exp_kRR==False:
                perturbed_poi_pre_direc = kRR(poi_angel_num_matrix_adap[perturbed_poi_list_for_dist[i]][this_poi], poi_angel_num_adap_list,epsilon_direction)
                perturbed_poi_post_direc = kRR(poi_angel_num_matrix_adap[this_poi][perturbed_poi_list_for_dist[i + 1]],poi_angel_num_adap_list,epsilon_direction)
                
                reversed_perturbed_poi_post_direc = get_reversed_num_direc_of_traj([perturbed_poi_post_direc], direc_num=len(poi_angel_num_adap_list))[0]
                
                this_poi_pre_direc_support_array = find_this_direc_support_of_this_poi(perturbed_poi_list_for_dist[i], perturbed_poi_pre_direc, poi_angel_num_matrix_adap)
                this_poi_post_direc_support_array = find_this_direc_support_of_this_poi(perturbed_poi_list_for_dist[i + 1], reversed_perturbed_poi_post_direc, poi_angel_num_matrix_adap)
                ...
                this_poi_direc_support_array = np.intersect1d(this_poi_pre_direc_support_array,this_poi_post_direc_support_array)
            else:
                if i==0:
                    
                    perturbed_poi_post_direc = kRR(poi_angel_num_matrix_adap[this_poi][perturbed_poi_list_for_dist[i]],poi_angel_num_adap_list,epsilon_direction)
                    
                    reversed_perturbed_poi_post_direc = get_reversed_num_direc_of_traj([perturbed_poi_post_direc], direc_num=len(poi_angel_num_adap_list))[0]
                    this_poi_direc_support_array = find_this_direc_support_of_this_poi(perturbed_poi_list_for_dist[i], reversed_perturbed_poi_post_direc, poi_angel_num_matrix_adap)
                    ...
                    perturbed_poi_pre_direc = kRR(poi_angel_num_matrix_adap[perturbed_poi_list_for_dist[-1]][this_poi],poi_angel_num_adap_list,epsilon_direction)
                    
                    this_poi_direc_support_array = find_this_direc_support_of_this_poi(perturbed_poi_list_for_dist[-1], perturbed_poi_pre_direc, poi_angel_num_matrix_adap)
            ...
            if len(this_poi_dist_support_array)>0:
                ...
                x=find_this_direc_support_of_this_poi(perturbed_poi_list_for_dist[i - 1], perturbed_poi_pre_direc, poi_angel_num_matrix_adap)
                y=np.intersect1d(mask,x).reshape(-1)
                if len(y>0):
                    
                    delta_of_this_poi_direc = delta_poi_distance_matrix
                    
                    over = np.exp(epsilon_per_poi * ( - poi_distance_matrix[this_poi][y]).reshape(-1) / (2 * delta_of_this_poi_direc))
        ...
        for i, this_poi in enumerate(poi_list_for_direc_dist):
            if kRR_exp_kRR==False:
                if i != len(poi_list_for_direc_dist) - 1:
                    ...
                    this_poi_direc_support_array = find_this_direc_support_of_this_poi(perturbed_poi_list_for_dist[i], perturbed_poi_pre_direc, poi_angel_num_matrix_adap)
                    
                    this_poi_direc_support_array=np.intersect1d(mask,this_poi_direc_support_array).reshape(-1)
                    
            else:
                if i==0:
                    perturbed_poi_post_direc = kRR(poi_angel_num_matrix_adap[perturbed_poi_list_for_dist[i]][this_poi],poi_angel_num_adap_list,epsilon_direction)
                    
                    this_poi_direc_support_array = find_this_direc_support_of_this_poi(perturbed_poi_list_for_dist[i], perturbed_poi_post_direc, poi_angel_num_matrix_adap)
                    
                    this_poi_direc_support_array=np.intersect1d(mask,this_poi_direc_support_array).reshape(-1)
                    
                else:
                    perturbed_poi_pre_direc = kRR(poi_angel_num_matrix_adap[perturbed_poi_list_for_dist[i - 1]][this_poi],poi_angel_num_adap_list,epsilon_direction)
                    perturbed_poi_post_direc = kRR(poi_angel_num_matrix_adap[this_poi][perturbed_poi_list_for_dist[i]],poi_angel_num_adap_list,epsilon_direction)
                    
                    reversed_perturbed_poi_post_direc = get_reversed_num_direc_of_traj([perturbed_poi_post_direc], direc_num=len(poi_angel_num_adap_list))[0]
                    
                    this_poi_pre_direc_support_array = find_this_direc_support_of_this_poi(perturbed_poi_list_for_dist[i - 1], perturbed_poi_pre_direc, poi_angel_num_matrix_adap)
                    this_poi_post_direc_support_array = find_this_direc_support_of_this_poi(perturbed_poi_list_for_dist[i], reversed_perturbed_poi_post_direc, poi_angel_num_matrix_adap)
...
def perturb_traj(traj, epsilon, poi_angel_num_matrix_adap, poi_angel_num_adap_list, mechanism='Exp_mechanism', kRR_exp_kRR=False,traj_idx=None):
    if mechanism == 'ATP_mechanism':
        perturb_mechanism = ATP_mechanism
    else:
        print('please choose a correct mechanism.')
    
    perturbed_traj = perturb_mechanism(traj, epsilon, poi_angel_num_matrix_adap, poi_angel_num_adap_list, kRR_exp_kRR,traj_idx=traj_idx)
...
for k in range(5):
    ...
    for eps_idx,epsilon in enumerate(epsilon_list):
        ...
        for i, trajectory in enumerate(traj_list_without_timestamp):
            ...
            perturbed_traj, _ = perturb_traj(trajectory, epsilon / 2, poi_angel_num_matrix_adap, poi_angel_num_adap_list, mechanism = 'ATP_mechanism', traj_idx = i)
            perturbed_reversed_traj, _ = perturb_traj(reversed_trajectory, epsilon / 2, poi_angel_num_matrix_adap, poi_angel_num_adap_list, mechanism = 'ATP_mechanism', kRR_exp_kRR = True, traj_idx = i)
            ...
            errors_px_of_perturbed_traj = get_preservation_range_queries_errors_of_perturbed_traj(trajectory, perturbed_traj)
